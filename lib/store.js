...
class Store extends EventEmitter {
  ...
  async getMany (keys = []) {
    ...
    const results = await recordFind(name, filter, { noCount: true })
    const values = []
    for (const k of keys) {
  ...
  async set (key, value, ttl) {
    const { importPkg } = this.scope.bajo.helper
    const { get, isPlainObject, isArray, map } = await importPkg('lodash-es')
    let coll
    let oids
    try {
  ...
  async clear () {
    const { recordClear } = this.scope.bajoDb.helper
    await recordClear(name)
    return true
  }
  ...
  async clearExpired () {
    ...
    const items = await recordFind(name, filter, { noCount: true })
  ...
  async has (key) {
    const { recordCount } = this.scope.bajoDb.helper
    const filter = {
      query: { id: key }
    }
    const result = await recordCount(name, filter)
    return result > 0
  }
}
