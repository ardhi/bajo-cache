...
class Store extends EventEmitter {
  ...
  set = async (key, value, ttl = 0) => {
    ...
    try {
      ...
      const token = Buffer.from(args.join('|'), 'base64').toString('utf8')
      const item = JSON.parse(token)
      const fields = ['model']
      for (const field of fields) {
        if (isSet(item[field])) body[field] = item[field]
      }
      let ids
      const values = get(JSON.parse(value), 'value.data')
      if (isArray(values)) ids = map(values, 'id')
      else if (isPlainObject(values)) ids = [values.id]
      if (ids && ids.length > 0) body.ids = `|${ids.join('|')}|`
    } catch (err) {}
